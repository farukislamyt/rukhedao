...
export function ModerationActionPanel({
  ...
  const allowedTransitions = useMemo(
    () => getAllowedIncidentStatusTransitions(incident.status),
    [incident.status],
  );
  ...
  async function handleStatusChange(e: React.FormEvent) {
    ...
    if (!reason) {
      setStatusMessage({ type: "error", text: t("reasonRequired") });
      return;
    }
  ...
  async function handleVerificationChange(e: React.FormEvent) {
    ...
    if (!reason) {
      setVerificationMessage({ type: "error", text: t("reasonRequired") });
      return;
    }
  ...
  async function handleEditSubmit(e: React.FormEvent) {
    ...
    if (!reason) {
      setEditMessage({ type: "error", text: t("reasonRequired") });
      return;
    }
    if (title.length < 5 || title.length > 200) {
      setEditMessage({ type: "error", text: t("invalidEditTitle") });
      return;
    }
    if (description.length < 20 || description.length > 10000) {
      setEditMessage({ type: "error", text: t("invalidEditDescription") });
      return;
    }
    if (!editDate || editDate > new Date().toISOString().slice(0, 10)) {
      setEditMessage({ type: "error", text: t("invalidEditDate") });
      return;
    }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-500">
              Current: <span className="font-semibold text-zinc-800 dark:text-zinc-200">{t(STATUS_LABEL_KEYS[incident.status])}</span>
            </p>
          </div>
          <span className="rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-[11px] font-semibold text-zinc-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
            {allowedTransitions.length} next {allowedTransitions.length === 1 ? "action" : "actions"}
          </span>
        </div>

        {allowedTransitions.length === 0 ? (
          <div className="mt-5 rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-xs text-zinc-600 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-400">
            This incident has no further workflow transition available from its current status.
          </div>
        ) : (
          <form onSubmit={handleStatusChange} className="mt-5 space-y-4">
            ...
            <button
              ...
              className="h-10 rounded-xl bg-zinc-950 px-4 text-xs font-semibold text-white transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-40 dark:bg-white dark:text-zinc-950"
            >
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500">
            Current: <span className="font-semibold text-zinc-800 dark:text-zinc-200">{t(VERIFICATION_LABEL_KEYS[incident.verification_status])}</span>
          </p>
        </div>
        ...
        <form onSubmit={handleVerificationChange} className="mt-5 space-y-4">
          <div>
            ...
            <select
              ...
              className="mt-1 h-11 w-full rounded-xl border border-zinc-300 bg-white px-3 text-sm outline-none focus:border-zinc-950 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white"
            >
              {(Object.keys(VERIFICATION_LABEL_KEYS) as VerificationStatus[]).map((value) => (
                <option key={value} value={value}>{t(VERIFICATION_LABEL_KEYS[value])}</option>
              ))}
            </select>
          </div>
        ...
        {isEditing && (
          <form onSubmit={handleEditSubmit} className="mt-5 space-y-4 border-t border-zinc-200 pt-5 dark:border-zinc-800">
            ...
            <div>
              ...
              <textarea
                required rows={7} minLength={20} maxLength={10000}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <input
                  type="date" required value={editDate}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    const nextDivision = Number(e.target.value);
                    setEditDivisionId(nextDivision);
                    const firstDistrict = districts.find((district) => district.division_id === nextDivision);
                    if (firstDistrict) setEditDistrictId(firstDistrict.id);
                  }}
                  className="mt-1 h-11 w-full rounded-xl border border-zinc-300 bg-white px-3 text-sm outline-none focus:border-zinc-950 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white"
                >
            ...
            <div>
              ...
              <input
                required maxLength={2000} value={editReason}
            ...
            <button
              ...
              className="h-10 rounded-xl bg-zinc-950 px-4 text-xs font-semibold text-white transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-40 dark:bg-white dark:text-zinc-950"
            >
              {editLoading ? "Saving…" : "Save revision"}
            </button>
          </form>
        )}
      </section>
