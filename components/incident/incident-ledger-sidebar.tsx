...
function Ledger({ title, type, items }: { title: string; type: "division" | "category"; items: LedgerItem[] }) {
    return (
        <div className="border border-zinc-200/90 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <div className="mt-2 divide-y divide-zinc-100 dark:divide-zinc-800">
                {items.length > 0 ? items.map((item) => (
                    <div key={`${type}-${item.name}`} className="flex items-center justify-between gap-3 py-2">
