...
function formatIncidentDate(value: string | null, locale: string) {
    if (!value) return "";
                                ...
                                {incidents.length > 0 ? (
                                    <div className="grid gap-5 md:grid-cols-2">
                                        {incidents.map((incident) => (
                                            <Link key={incident.public_id} href={`/incidents/${incident.public_id}`} className="group rounded-2xl border border-zinc-200 bg-stone-50 p-6 transition hover:-translate-y-0.5 hover:border-zinc-300 hover:bg-white hover:shadow-lg">
                                                <div className="flex items-center justify-between gap-4">
                                                    <span className="truncate rounded-full border border-zinc-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-zinc-600">{incident.category ?? t("sampleCategory")}</span>
                                                    <span className="shrink-0 font-mono text-[10px] text-zinc-400">{incident.public_id}</span>
                                                </div>
                                                <h3 className="mt-7 line-clamp-3 text-lg font-semibold leading-7 group-hover:underline group-hover:decoration-zinc-300 group-hover:underline-offset-4">{incident.title ?? t("sampleIncidentTitle")}</h3>
                                                <p className="mt-3 line-clamp-2 text-sm leading-6 text-zinc-500">{incident.description ?? ""}</p>
                                                <div className="mt-8 flex items-center justify-between border-t border-zinc-200 pt-4 text-xs text-zinc-500"><span>{[incident.district, incident.division].filter(Boolean).join(", ")}</span><span>{formatIncidentDate(incident.incident_date, locale)}</span></div>
                                            </Link>
                                        ))}
                                    </div>
                                ) : (
                                    <div className="border border-dashed border-zinc-300 bg-stone-50 px-6 py-14 text-center"><p className="text-sm text-zinc-500">{t("noRecentIncidents")}</p></div>
                                )}
                            </div>
